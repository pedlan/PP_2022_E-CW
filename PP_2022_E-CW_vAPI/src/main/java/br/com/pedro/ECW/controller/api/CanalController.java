...
public class CanalController {
    ...
    @PostMapping("/{id}/txon")
    public ResponseEntity<Canal> txOn(@PathVariable Long id) {
        Optional<Canal> canalOp = canalService.findById(id);
        if (!canalOp.isPresent()) {
            return ResponseEntity.notFound().build();
        }
        int tx = canalOp.get().getTx();
        tx++;
        Canal c = new Canal(canalOp.get(), tx);
        c = canalService.save(c);
        return ResponseEntity.ok().body(c);
    }

    @PostMapping("/{id}/txoff")
    public ResponseEntity<Canal> txOff(@PathVariable Long id) {
        Optional<Canal> canalOp = canalService.findById(id);
        if (!canalOp.isPresent()) {
            return ResponseEntity.notFound().build();
        }
