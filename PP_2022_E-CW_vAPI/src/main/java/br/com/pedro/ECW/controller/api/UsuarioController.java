...
public class UsuarioController {
    ...
    public ResponseEntity<Usuario> authenticate(@PathVariable String email, @PathVariable String senha) {

        List<Usuario> usuarioList = usuarioService.findALl();

        // check nenhum usuário cadastrado
        if (usuarioList.size() == 0) {
            return null;
        }

        Usuario u = usuarioList.stream().filter(usuario -> email.equals(usuario.getEmail())).findFirst().orElse(null);

        // check usuário não encontrado
        if (u == null) {
            return null;
        }

        String hash = sha256(senha + u.getSalt());

        // check senha
        if (u.getSenha().equals(hash)) {
